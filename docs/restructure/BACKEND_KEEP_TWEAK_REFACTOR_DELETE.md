...
| Specializations + legacy VC router | Delete or defer | `backend/routers/specializations.py`, `backend/api/vc_routes.py`, `backend/modules/specializations/*` | New product is general clone-for-experts; the VC-only router is already retired from active runtime registration. |
