...
### Legacy VC-only routes
`backend/api/vc_routes.py` and the `ENABLE_VC_ROUTES` gate are no longer part of the current runtime. Keep any older references as historical context only.
