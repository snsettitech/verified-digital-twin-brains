...
### Historical note: dedicated VC-only router
No `backend/api/vc_routes.py` file is currently registered in `backend/main.py`.
VC-specialized twins use the shared specialization routes instead of a separate `/api/vc/*` router surface.
