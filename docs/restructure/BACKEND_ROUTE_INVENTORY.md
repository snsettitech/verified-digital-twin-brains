...
### Historical Note: VC Router
The live backend no longer mounts `backend/api/vc_routes.py`. VC-specific behavior now runs through the shared specialization registry and `backend/routers/specializations.py`.
