...
### Historical note: `backend/api/vc_routes.py`
The legacy VC-only `/api/vc/*` router is no longer registered in `backend/main.py`. Current specialization behavior flows through the shared specialization endpoints already listed above.
