...
- Historical VC-specific route docs may still reference `backend/api/vc_routes.py`, but the active backend surface no longer mounts that router module.
- Uvicorn entrypoints:
...
### Historical: `backend/api/vc_routes.py`
This route module is documented for reference only. The active backend surface
in this repository no longer mounts it.
| Method | Path | Request | Response | Auth/Deps |
| --- | --- | --- | --- | --- |
