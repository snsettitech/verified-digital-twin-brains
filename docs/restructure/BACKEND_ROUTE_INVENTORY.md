...
### VC routes
`backend/main.py` no longer registers a standalone VC-only router. Historical references to `backend/api/vc_routes.py` in older design docs describe a removed path, not a current runtime surface.
