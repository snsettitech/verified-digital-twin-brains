...
- Files:
  ...
  - `backend/api/vc_routes.py` (legacy VC-specific route module)
...
- Why (evidence): Product is single "expert" path. Specializations are optional and legacy VC-only routes still exist in the codebase.
...
- Verify unused before delete:
  ...
  - Ensure no `/api/vc` routes remain registered before removing specialization APIs from `backend/main.py`.
