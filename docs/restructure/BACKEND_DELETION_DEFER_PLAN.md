...
### Specializations + historical VC route surface (DEFER then DELETE)
- Files:
...
- Why (evidence): Product is single "expert" path. Shared specialization surfaces are still present, while the dedicated VC-only router has already been retired from `backend/main.py`.
...
- Verify unused before delete:
  ...
  - Ensure the retired VC-only route surface stays absent and shared specialization routes still match product needs.
