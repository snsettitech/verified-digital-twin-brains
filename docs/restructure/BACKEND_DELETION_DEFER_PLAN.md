...
- Why (evidence): Product is single "expert" path. Specializations are optional, and the legacy VC-only route surface has already been removed.
...
- Verify unused before delete:
  ...
  - Ensure no dedicated VC-only router or startup gate reappears in `backend/main.py`.
