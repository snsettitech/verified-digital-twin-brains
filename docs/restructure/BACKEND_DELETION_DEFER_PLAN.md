...
- Files:
  ...
  - `backend/api/vc_routes.py` (historical reference; current `backend/main.py` no longer wires `ENABLE_VC_ROUTES`)
...
- Why (evidence): Product is single "expert" path. Specializations are optional and VC route docs remain as historical design material, even though the old env gate has been removed from `backend/main.py`.
...
- Verify unused before delete:
  ...
  - Confirm no live router registration remains for VC-only endpoints and `app.include_router(specializations.router)` can be removed from `backend/main.py` when specialization support is retired.
