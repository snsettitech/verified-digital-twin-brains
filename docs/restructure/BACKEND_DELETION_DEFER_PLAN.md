...
- Why (evidence): Product is single "expert" path. Specializations are optional, and the old dedicated VC router has already been removed from the live app surface.
...
- Verify unused before delete:
  ...
  - Ensure no `/api/vc` route surface remains and `app.include_router(specializations.router)` can eventually be removed from `backend/main.py`.
