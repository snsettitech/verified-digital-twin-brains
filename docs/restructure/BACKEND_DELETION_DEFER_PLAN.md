...
- Why (evidence): Product is single "expert" path. Specializations are optional; the current backend tree no longer contains a dedicated `backend/api/vc_routes.py` module.
...
- Verify unused before delete:
  ...
  - Ensure obsolete VC route docs are removed and `app.include_router(specializations.router)` is still intentional in `backend/main.py`.
