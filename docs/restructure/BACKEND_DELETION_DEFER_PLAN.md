...
- Files:
  ...
  - `backend/api/vc_routes.py` (already removed from the live app)
...
- Why (evidence): Product is single "expert" path. Specializations are optional, and the old VC-only route file has already been removed from the live app surface.
...
- Verify unused before delete:
  ...
  - Confirm no VC-only route wiring remains in `backend/main.py` and `backend/api/vc_routes.py` is not referenced by live code.
