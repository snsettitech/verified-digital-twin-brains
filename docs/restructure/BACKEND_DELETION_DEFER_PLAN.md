...
- Why (evidence): Product is single "expert" path. Specializations are optional, and the old dedicated VC-only router surface is no longer mounted in `backend/main.py`.
...
- Verify unused before delete:
  ...
  - Ensure no dedicated VC-only router surface remains and `app.include_router(specializations.router)` can be removed from `backend/main.py` when specialization support is retired.
