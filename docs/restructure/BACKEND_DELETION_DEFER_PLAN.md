...
- Why (evidence): Product is single "expert" path. Specializations are optional, and older planning docs referenced a VC-only router that is no longer wired in the live backend.
...
- Verify unused before delete:
  ...
  - Confirm no legacy VC-only route assumptions remain in active docs or tests.
