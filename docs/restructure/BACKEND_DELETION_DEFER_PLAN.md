...
- Files:
  ...
  - `backend/api/vc_routes.py` (present in repo but not registered by `backend/main.py`)
...
- Why (evidence): Product is single "expert" path. Specializations are optional and VC routes remain in the repo even though runtime registration has already been removed from `backend/main.py`.
...
- Verify unused before delete:
  ...
  - Ensure `backend/api/vc_routes.py` remains unregistered and `app.include_router(specializations.router)` can be removed safely from `backend/main.py`.
