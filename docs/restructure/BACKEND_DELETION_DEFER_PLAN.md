...
- Why (evidence): Product is single "expert" path. Specializations are optional, and the former VC-only route gate has already been removed from the live backend.
...
- Verify unused before delete:
  ...
  - Ensure specialization-only surfaces are unused before removing them from `backend/main.py`.
