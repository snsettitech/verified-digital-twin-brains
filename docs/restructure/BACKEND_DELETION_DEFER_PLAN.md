...
- Why (evidence): Product is single "expert" path. Specializations are optional and the old VC-only upload route has already been removed from the live app surface.
...
- Verify unused before delete:
  ...
  - Ensure no VC-only router is referenced and `app.include_router(specializations.router)` is still the only specialization route surface in `backend/main.py`.
