...
- Conditional routers: `enhanced_ingestion` is gated by `ENABLE_ENHANCED_INGESTION`. Historical VC docs may still mention `api/vc_routes.py`, but the active backend surface no longer mounts that router module.
