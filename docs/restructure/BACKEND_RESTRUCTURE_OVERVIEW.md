...
- Conditional routers: `enhanced_ingestion` gated by `ENABLE_ENHANCED_INGESTION` in `backend/main.py`. VC-specialized twins now use the shared specialization routes rather than a dedicated `api/vc_routes.py` router.
