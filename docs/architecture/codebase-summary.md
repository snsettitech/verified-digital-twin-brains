...
│   │   └── shared specialization routes # Includes VC specialization flows
...
- **Routes**: Shared specialization endpoints (no VC-only router)
...
- ✅ **Specialization routes**: Shared router stays mounted without a VC-only env gate
...
2. **Router Count**: Verify against the current `backend/main.py` router list instead of assuming a VC-only conditional router
3. **Module Count**: 33 modules (not 25+)
4. **P0-P1 Hardening**: Add section on recent reliability/security improvements
5. **VC Specialization**: Mention shared specialization routes and lazy VC loading
