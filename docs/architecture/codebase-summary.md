...
- **Routes**: Served through the shared specialization/router surfaces
...
- ✅ **VC specialization**: Uses shared application routing instead of a separate VC-only route module
...
2. **Router Count**: 17 routers (not 16) - excludes the previously documented VC-only route module
3. **Module Count**: 33 modules (not 25+)
4. **P0-P1 Hardening**: Add section on recent reliability/security improvements
5. **VC Specialization**: Clarify that VC behavior lives in shared specialization routing
