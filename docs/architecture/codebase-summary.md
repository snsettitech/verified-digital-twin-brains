...
- **Routes**: No dedicated VC-only router in the current runtime
...
- ✅ **VC route cleanup**: Legacy VC-only route gating removed from `backend/main.py`
...
| **VC Routes** | Not mentioned | Removed from current runtime | ✅ Historical route cleaned up |
...
5. **VC Routes**: Remove stale references to legacy VC-only route gating
