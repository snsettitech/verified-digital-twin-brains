...
- **Routes**: No dedicated VC-only router in the current backend tree
...
- ✅ **VC specialization loading**: Lazy module loading keeps VC logic isolated from the core path
...
5. **VC Specialization**: Clarify that VC logic is specialization-driven, not routed via a dedicated `vc_routes` module
