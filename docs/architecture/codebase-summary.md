...
| **vc** (not registered) | `api/vc_routes.py` | `/api/vc/artifact/upload/{twin_id}` | VC-specific route module kept in-repo for historical context, but not registered by `backend/main.py`. |
...
- **Routes**: Module exists, but no VC-specific routes are currently registered in `backend/main.py`
...
- ✅ **VC route module**: Kept in-repo, but not mounted by the current FastAPI app
...
2. **Router Count**: Recount from `backend/main.py` instead of assuming legacy VC-only routes are mounted
3. **Module Count**: 33 modules (not 25+)
4. **P0-P1 Hardening**: Add section on recent reliability/security improvements
5. **VC Routes**: Mention that the `api/vc_routes.py` module exists but is not mounted by the current app
