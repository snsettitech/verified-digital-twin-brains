...
| **vc** (historical) | `api/vc_routes.py` | `/api/vc/artifact/upload/{twin_id}` | Historical VC-specific route design. No live router registration remains in `backend/main.py`. |
...
- **Routes**: Historical docs only; no live VC route registration remains in `backend/main.py`
...
- ✅ **VC route docs**: Preserved as historical design references; no live VC route registration remains in `backend/main.py`
...
5. **VC Routes**: Note that VC route docs are historical and not wired in the current backend startup path
