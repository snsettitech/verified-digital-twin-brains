...
- **Routes**: Uses the shared specialization endpoints in `backend/routers/specializations.py`
...
- ✅ **VC specialization**: Uses the shared specialization router; no dedicated `/api/vc` route surface remains
...
|--------|--------------|---------------|--------|
| **Routers** | 16 | Route inventory maintained in `backend/main.py` and `docs/restructure/BACKEND_ROUTE_INVENTORY.md` | ⚠️ Minor update |
...
| **VC Specialization** | Not mentioned | Uses shared specialization routes | ⚠️ New feature |
...
2. **Router Count**: Verify against `backend/main.py` and `docs/restructure/BACKEND_ROUTE_INVENTORY.md`
3. **Module Count**: 33 modules (not 25+)
4. **P0-P1 Hardening**: Add section on recent reliability/security improvements
5. **VC Specialization**: Mention the shared specialization route surface instead of a dedicated VC router
