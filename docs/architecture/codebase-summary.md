...
- **Routes**: Shared specialization config endpoints in `backend/routers/specializations.py`
...
- ✅ **Specialization routing**: Shared config endpoints stay mounted even when no VC-specific router exists
...
|--------|--------------|---------------|--------|
| **Routers** | 16 | 16 live routers in this summary slice | ⚠️ Minor update |
...
| **VC Specialization** | Not mentioned | Shared specialization config endpoints | ⚠️ New feature |
...
2. **Router Count**: Refresh the count from live `backend/routers/*` rather than historical VC-only routing notes
3. **Module Count**: 33 modules (not 25+)
4. **P0-P1 Hardening**: Add section on recent reliability/security improvements
5. **VC Specialization**: Mention the shared specialization config endpoints instead of a dedicated VC router
