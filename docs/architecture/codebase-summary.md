...
- **Routes**: Uses the shared specialization APIs; no dedicated VC router is registered
...
- ✅ **VC specialization**: Lazy loading keeps VC-specific logic off the default route surface
...
2. **Router Count**: Route registration in `backend/main.py` is the canonical source of truth
3. **Module Count**: 33 modules (not 25+)
4. **P0-P1 Hardening**: Add section on recent reliability/security improvements
5. **VC Specialization**: Note that it now relies on shared specialization routes only
