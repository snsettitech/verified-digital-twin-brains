...
│   │   └── profile.py        # Profile abstraction routes
...
- **Routes**: Uses shared `specializations` routes; no dedicated `/api/vc` router is registered in the current backend
...
- ✅ **VC specialization**: Uses shared specialization routes and lazy loading so VC assets do not affect default flows
...
2. **Router Count**: 16 core routers plus additional active product/observability routers registered in `backend/main.py`
3. **Module Count**: 33 modules (not 25+)
4. **P0-P1 Hardening**: Add section on recent reliability/security improvements
5. **VC specialization**: Mention lazy loading via shared specialization routes rather than a dedicated VC router
