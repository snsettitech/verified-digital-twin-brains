...
| **import_routes** | `import_routes.py` | `/import/linkedin/export`, `/import/linkedin/preview` | LinkedIn/data import endpoints. |
...
- **Routes**: No dedicated VC router is currently registered in `backend/main.py`
...
- ✅ **VC specialization**: Lazy specialization loading with vanilla fallback; no dedicated VC router is registered in `backend/main.py`
...
| **VC Runtime Surface** | Not mentioned | No dedicated VC router; specialization support is lazy-loaded | ⚠️ Updated architecture |
...
5. **VC Specialization Runtime**: Mention lazy specialization loading and lack of a dedicated VC router
