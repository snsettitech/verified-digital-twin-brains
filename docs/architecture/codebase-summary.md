...
- **Routes**: Uses the shared specialization route surface in `backend/main.py`
...
- ✅ **VC route flag cleanup**: Removed stale `ENABLE_VC_ROUTES` references after the dedicated VC router path was deleted
...
5. **Registry Loader**: Mention vanilla fallback logic
6. **Retrieval**: Mention P1-C timeouts (2s/5s/3s)
7. **LangGraph**: Mention P1-A Postgres checkpointer
