...
- **Routes**: Uses shared specialization surfaces; no dedicated VC-only router is registered
...
- ✅ **Specialization routing**: Shared specialization surfaces stay registered without a separate VC-only router
...
2. **Router Count**: 17 routers (not 16) - excludes retired VC-only route wiring
3. **Module Count**: 33 modules (not 25+)
4. **P0-P1 Hardening**: Add section on recent reliability/security improvements
5. **VC Routing**: Note that VC behavior now relies on shared specialization routes rather than a dedicated VC-only router
