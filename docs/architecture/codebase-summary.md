...
- **Runtime**: Requests normalize to the shared vanilla specialization path
- **Routes**: No dedicated VC router is registered in the live backend
...
2. **Router Count**: Update from exact historical counts before relying on them
3. **Module Count**: 33 modules (not 25+)
4. **P0-P1 Hardening**: Add section on recent reliability/security improvements
5. **VC Routes**: Treat VC route docs as historical unless a live router is reintroduced
