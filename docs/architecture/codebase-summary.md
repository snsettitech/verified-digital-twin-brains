...
- **Routes**: No dedicated VC-only router currently mounted
- **Loading**: Lazy specialization loading only when `specialization_id='vc'`
...
- ✅ **VC specialization**: Registry-only loading keeps VC logic from affecting vanilla startup
...
2. **Router Count**: Update against the live `backend/main.py` router list, not historical VC route docs
3. **Module Count**: 33 modules (not 25+)
4. **P0-P1 Hardening**: Add section on recent reliability/security improvements
5. **VC Specialization**: Mention lazy specialization loading rather than a separate VC route flag
