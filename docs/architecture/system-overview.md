...
- **Issues**:
  ...
  - VC specialization behavior still depends on shared specialization routes and registry config
  
**Evidence**:
- `modules/specializations/registry.json` exists
- `registry_loader.py` has fallback pattern
- Shared specialization routes remain mounted from `routers/specializations.py`
