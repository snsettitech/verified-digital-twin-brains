...
- **Issues**:
  ...
  - VC specialization remains lazy-loaded, but the legacy VC-only route flag has been removed
  
**Evidence**:
- `modules/specializations/registry.json` exists
- `registry_loader.py` has fallback pattern
- VC specialization still resolves through the registry without dedicated VC-only route wiring
