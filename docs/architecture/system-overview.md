...
- **Issues**:
  ...
  - Historical VC-route docs are stale; no VC-specific route gate exists in the current backend
  
**Evidence**:
- `modules/specializations/registry.json` exists
- `registry_loader.py` has fallback pattern
- Shared specialization routes register unconditionally in `backend/main.py`
