...
- [x] Remove the obsolete opt-out flags once the stable routes are fully launched.
...
- [x] Confirm legacy disable env vars no longer affect stable route registration.
...
- Risk: Existing deployments rely on current disabled defaults.
  Mitigation: Document behavior change and verify the stable routes stay mounted.
