-- Phase 3 Migration: Claims, Contradictions, and Snapshots (FIXED)
-- Creates tables for claim extraction, contradiction review queue, and materialized snapshots
-- 
-- FIX: Cast auth.jwt()->>'tenant_id' to UUID for proper type matching
...
-- FIX: Cast JWT result to UUID
DROP POLICY IF EXISTS "tenant_isolation_contradiction_queue" ON graph_contradiction_queue;
...
-- FIX: Cast JWT result to UUID
DROP POLICY IF EXISTS "tenant_isolation_snapshots" ON graph_context_snapshots;
...
-- FIX: Cast JWT result to UUID
DROP POLICY IF EXISTS "tenant_isolation_claims" ON graph_claims;
