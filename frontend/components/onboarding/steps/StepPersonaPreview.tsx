...
interface StepPersonaPreviewProps {
  twinId: string | null;
  onActivate: () => void;
}

export function StepPersonaPreview({ twinId, onActivate }: StepPersonaPreviewProps) {
  ...
  const handleActivate = async () => {
    setActivating(true);
    try {
      // Update twin name if changed
      if (twinName && twinId) {
        await authFetchStandalone(`/twins/${twinId}/activate`, {
          method: 'POST',
  ...
  return (
    <div className="space-y-6">
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold mb-2">Your Persona is Ready!</h2>
